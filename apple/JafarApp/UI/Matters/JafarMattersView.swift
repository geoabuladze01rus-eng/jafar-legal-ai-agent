...
struct JafarMatterWorkspaceView: View {
    ...
    var body: some View {
        ScrollView { VStack(alignment: .leading, spacing: 20) {
            ...
            if tab == "Хронология" { MatterTimelineView(matterID: identifier) } else if tab == "Документы" { MatterDocumentsView(matterID: identifier) } else if tab == "Сроки" { MatterDeadlinesView(matterID: identifier) } else if tab == "Правовая позиция" { JafarLegalPositionView(matterID: identifier) } else { JafarCard(title: tab) { Text("Раздел будет заполнен после подключения источника данных.").foregroundStyle(JafarPalette.secondary) } }
