...
struct JafarMatterWorkspaceView: View {
    ...
    var body: some View {
        ScrollView { VStack(alignment: .leading, spacing: 20) {
            ...
            if tab == "Хронология" { MatterTimelineView(matterID: identifier) } else { JafarCard(title: tab) { Text("Раздел будет заполнен после подключения источника данных.").foregroundStyle(JafarPalette.secondary) } }
