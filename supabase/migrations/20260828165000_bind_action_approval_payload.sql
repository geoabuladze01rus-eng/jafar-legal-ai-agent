-- Compatibility timestamp for clients/tests that reference the original migration name.
-- The idempotent SQL is intentionally identical to 20260828165500.
alter table public.action_approvals add column if not exists payload_hash text null;
...
begin
  ...
  if old.state = 'proposed' and new.state = 'approved' then
    if old.payload_hash is null or btrim(old.payload_hash) = '' then raise exception 'payload_binding_required'; end if;
    return new;
  end if;
  if old.state = 'proposed' and new.state = 'rejected' then return new; end if;
