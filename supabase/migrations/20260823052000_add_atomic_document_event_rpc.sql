...
begin
  ...
  for v_deadline in select * from jsonb_array_elements(coalesce(p_deadlines, '[]'::jsonb)) loop
    ...
    values (p_matter_id, v_owner, v_deadline->>'title',
            (v_deadline->>'due_date')::timestamptz, v_deadline->>'source_text');
  end loop;

  insert into public.matter_events
    (matter_id, owner_user_id, title, event_date, description, source_document, document_fingerprint)
  values
    (p_matter_id, v_owner, p_title, p_event_date, p_description, p_source_document, p_document_fingerprint)
  returning * into v_event;

  update public.matters set updated_at = now() where id = p_matter_id;
  return v_event;
exception
...
revoke all on function public.record_document_event(uuid, text, timestamptz, text, text, text, jsonb) from public;
grant execute on function public.record_document_event(uuid, text, timestamptz, text, text, text, jsonb) to authenticated;
