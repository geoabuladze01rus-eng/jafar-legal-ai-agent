from jafar.attack_surface import AttackSeverity, ProsecutionAttackSurfaceEngine
from jafar.case_theory import TheoryStatus
from jafar.theory_views import DualTheoryReport, TheorySide, TheoryViewItem


def item(issue_id: str, topic: str, side: TheorySide, status: TheoryStatus, *, source: str, actor: str = "actor") -> TheoryViewItem:
...
def test_single_source_contradicted_thesis_with_defense_counterpoint_ranks_critical() -> None:
    prosecution = item("p1", "payment", TheorySide.PROSECUTION, TheoryStatus.CONTRADICTED, source="p-src")
    defense = item("d1", "payment", TheorySide.DEFENSE, TheoryStatus.SUPPORTED, source="d-src", actor="witness")
...
def test_supported_thesis_with_multiple_sources_has_lower_priority() -> None:
    prosecution = TheoryViewItem(
        ...
        evidence_ids=("s1", "s2"),
        source_refs=(
            {"evidence_id": "s1", "document_name": "a.pdf", "page": 1, "chunk_index": 0, "actor": "w1", "event_id": None, "excerpt": "a"},
            {"evidence_id": "s2", "document_name": "b.pdf", "page": 2, "chunk_index": 0, "actor": "w2", "event_id": None, "excerpt": "b"},
        ),
        reasons=(),
    )
    result = ProsecutionAttackSurfaceEngine().build(DualTheoryReport(
        prosecution=(prosecution,), defense=(), neutral=(), conflict_points=(), requires_human_review=False,
    ))
...
def test_unsupported_prosecution_thesis_is_high_priority_even_without_defense_counterpoint() -> None:
    ...
    result = ProsecutionAttackSurfaceEngine().build(DualTheoryReport(
        prosecution=(prosecution,), defense=(), neutral=(), conflict_points=(), requires_human_review=True,
    ))
...
def test_ranking_orders_more_vulnerable_thesis_first() -> None:
    weak = item("weak", "payment", TheorySide.PROSECUTION, TheoryStatus.CONTRADICTED, source="weak-src")
    ...
    result = ProsecutionAttackSurfaceEngine().build(DualTheoryReport(
        prosecution=(strong, weak), defense=(), neutral=(), conflict_points=(), requires_human_review=True,
    ))
