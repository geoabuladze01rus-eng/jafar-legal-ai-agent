    ...
    def build(self, report: DualTheoryReport) -> AttackSurfaceReport:
        ...
        for item in report.prosecution:
            ...
            if item.status == TheoryStatus.CONTRADICTED:
                ...
                focus.append("Сопоставить противоречащие источники и подготовить вопросы по расхождениям.")
                ...
                focus.append("Потребовать первичный источник и проверить допустимость его использования.")

            source_count = len({source.get("evidence_id") for source in item.source_refs if source.get("evidence_id")})
            document_count = len({source.get("document_name") for source in item.source_refs if source.get("document_name")})
            actor_count = len({source.get("actor") for source in item.source_refs if source.get("actor")})
            ...
            if document_count <= 1:
                score += 10
                reasons.append("Нет подтверждения из нескольких независимых документов.")
            if actor_count <= 1 and any(source.get("actor") for source in item.source_refs):
                score += 10
                reasons.append("Доказательственная база зависит от одного участника/источника показаний.")
                focus.append("Проверить устойчивость показаний и внешнюю подтверждаемость.")

            defense_sources = tuple(source for defense in defense_items for source in defense.source_refs)
            if defense_items:
                score += 25
                reasons.append("По той же теме имеется оформленный защитный контртезис.")
                focus.append("Сформировать прямой ответ на защитный контртезис и проверить, чем он подтверждён.")
