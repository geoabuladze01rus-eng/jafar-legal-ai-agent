...
def test_development_allows_local_api_without_key(monkeypatch) -> None:
    ...
    with TestClient(app) as client:
        response = client.post(
            "/v1/command",
            json={"text": "проверка связи", "user_id": "local", "source_device": "mac"},
        )
...
def test_configured_key_is_required_and_constant_time_checked(monkeypatch) -> None:
    ...
    payload = {"text": "проверка связи", "user_id": "local", "source_device": "mac"}

    with TestClient(app) as client:
        missing = client.post("/v1/command", json=payload)
        wrong = client.post("/v1/command", json=payload, headers={"X-Jafar-API-Key": "wrong"})
        valid = client.post("/v1/command", json=payload, headers={"X-Jafar-API-Key": "test-secret"})
