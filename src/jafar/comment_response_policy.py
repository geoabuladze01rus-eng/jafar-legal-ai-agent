...
def build_response(intent: CommentIntent, text: str) -> ResponseDecision:
    ...
    if intent is CommentIntent.LEGAL_HELP:
        return ResponseDecision("auto_reply", "В такой ситуации важны конкретные обстоятельства и документы. Мы не оказываем индивидуальную юридическую помощь в комментариях, но можем разобрать общий порядок действий.")
