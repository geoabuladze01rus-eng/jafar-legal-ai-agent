...
from jafar.action_approval import ActionRequest, ActionState, LegalActionApprovalEngine, payload_fingerprint
...
class FakeQuery:
    ...
    def select(self, _: str) -> "FakeQuery": return self
    def eq(self, field: str, value: Any) -> "FakeQuery":
        self.filters.append((field, value)); return self
    def maybe_single(self) -> "FakeQuery": self.single = True; return self
    def order(self, _: str) -> "FakeQuery": return self
    def insert(self, payload: Any) -> "FakeQuery": self.insert_payload = deepcopy(payload); return self
    def update(self, payload: dict[str, Any]) -> "FakeQuery": self.update_payload = deepcopy(payload); return self

    def execute(self) -> SimpleNamespace:
        rows = self.client.data.setdefault(self.table_name, [])
        if self.insert_payload is not None:
            inserted = deepcopy(self.insert_payload if isinstance(self.insert_payload, list) else [self.insert_payload])
            rows.extend(inserted)
            return SimpleNamespace(data=deepcopy(inserted))
        indexes = [i for i, row in enumerate(rows) if all(row.get(f) == v for f, v in self.filters)]
        if self.update_payload is not None:
            updated = []
            for index in indexes:
                rows[index].update(deepcopy(self.update_payload)); updated.append(deepcopy(rows[index]))
            return SimpleNamespace(data=updated)
        selected = [deepcopy(rows[i]) for i in indexes] if self.filters else deepcopy(rows)
...
class FakeSupabase:
    ...
    def table(self, name: str) -> FakeQuery: return FakeQuery(self, name)
...
def test_supabase_store_retains_full_claim_and_execution_history() -> None:
    client = FakeSupabase(); store = SupabaseActionApprovalRepository(client, "owner-1")
    engine = LegalActionApprovalEngine(store); request = make_request(); store.add(request)
...
def test_second_executor_cannot_claim_same_action() -> None:
    client = FakeSupabase(); store = SupabaseActionApprovalRepository(client, "owner-1")
    request = make_request("claim-once"); store.add(request)
...
def test_claim_release_requires_owner_and_preserves_retry_error() -> None:
    client = FakeSupabase(); store = SupabaseActionApprovalRepository(client, "owner-1")
    request = make_request("retry"); store.add(request)
...
def test_payload_hash_is_persisted_and_hydrated() -> None:
    client = FakeSupabase(); store = SupabaseActionApprovalRepository(client, "owner-1")
    request = make_request("payload-bound"); store.add(request); hydrated = store.get("payload-bound")
    assert hydrated is not None and hydrated.payload_hash == request.payload_hash


def test_unbound_action_cannot_transition_to_approved() -> None:
    client = FakeSupabase(); store = SupabaseActionApprovalRepository(client, "owner-1")
    request = make_request("unbound", bound=False); store.add(request)
...
def test_second_decision_cannot_overwrite_first_lawyer_decision() -> None:
    ...
    with pytest.raises(ValueError, match="action_not_pending"):
        store.decide("a1", state=ActionState.REJECTED, decided_by="lawyer:second", reason="Позднее")
...
def test_rejection_requires_reason_and_remains_queryable() -> None:
    ...
    rejected = store.decide("reject-1", state=ActionState.REJECTED, decided_by="lawyer", reason="Проверить")
    assert store.rejected() == (rejected,)


def test_store_is_strictly_owner_scoped() -> None:
    client = FakeSupabase(); one = SupabaseActionApprovalRepository(client, "owner-1"); two = SupabaseActionApprovalRepository(client, "owner-2")
    one.add(make_request("same-id")); two.add(make_request("same-id"))
    one.decide("same-id", state=ActionState.APPROVED, decided_by="lawyer:one")
    assert one.get("same-id").state is ActionState.APPROVED
    assert two.get("same-id").state is ActionState.PROPOSED
