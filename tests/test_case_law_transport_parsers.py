...
def fetched(body: str) -> HttpFetchResult:
    return HttpFetchResult(
        url="https://example.test/list-resolved",
        requested_url="https://example.test/list",
        status_code=200,
...
def test_parser_extracts_case_law_links_and_provenance() -> None:
    ...
    assert page.provenance.requested_url == "https://example.test/list"
    assert page.provenance.resolved_url == "https://example.test/list-resolved"


def test_parser_extracts_russian_month_date() -> None:
    parser = LinkBasedCaseLawParser(
        source_name="sudact",
    ...
    page = parser.parse(
        fetched(
            '<a href="/vsrf/doc/1">'
            "Определение от 16 августа 2026 г. по делу № 18-КГ26-1"
            "</a>"
        )
    )
    assert page.items[0].decided_on == date(2026, 8, 16)
