...
struct CanonicalHomePrototypeView: View {
    ...
    private var topMainGrid: some View {
        HStack(alignment: .top, spacing: 10) {
            ...
            CanonicalPanel(glow: JafarPalette.goldGlow) { ZStack { Circle().fill(JafarPalette.goldGlow.opacity(0.16)).blur(radius: 22).scaleEffect(haloBreath ? 1.07 : 0.92); Circle().stroke(JafarPalette.accent.opacity(0.45), lineWidth: 1).frame(width: 190, height: 190).rotationEffect(.degrees(ringTurn ? 360 : 0)); Image("JusticeHero").resizable().scaledToFill().frame(maxWidth: .infinity, maxHeight: 214).clipped().mask(LinearGradient(colors: [.clear, .white, .white, .clear], startPoint: .leading, endPoint: .trailing)); LinearGradient(colors: [.clear, JafarPalette.goldHighlight.opacity(0.12), .clear], startPoint: .topLeading, endPoint: .bottomTrailing).frame(width: 90).offset(x: sweepPhase ? 190 : -190).blur(radius: 8); VStack(alignment: .leading) { Spacer(); Text("ЮСТИЦИЯ").font(JusticeTypography.titleLarge); Text("Система активна").font(.caption2).foregroundStyle(JafarPalette.accentGold) }.frame(maxWidth: .infinity, alignment: .leading) }.frame(maxWidth: .infinity, maxHeight: .infinity) }.frame(maxWidth: .infinity, minHeight: 238)
